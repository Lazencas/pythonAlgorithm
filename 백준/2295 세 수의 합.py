...
# 입력
# 첫째 줄에 자연수 N이 주어진다. 다음 N개의 줄에 차례로 U의 원소가 하나씩 주어진다. 주어진 U는 집합이 되므로 
# 입력되는 두 수가 같아서는 안 된다. U의 원소는 200,000,000보다 작거나 같은 자연수이다. 답이 항상 존재하는 경우만
#  입력으로 주어진다.
...
'''
3가지 수를 골라서, 세 수의 합d가 U안에 포함되는 경우중에 가장 큰 경우의 d

3가지 수를 골라서 그 합 d가 U안에 있는 모든 경우의 수를 따지면 되지 않나?
입력 1000개 이하니까..... 시간제한1초면 이중반복문 + 적당한 반복문 가능
U는 집합. 2억보다 작거나 같은 자연수
